...
import taskautomation.rule.RuleSingleExecute;
import taskautomation.rule.RuleWithSleep;
...
public class FXMLDocumentAddRuleController implements Initializable {
    ...
    private void createRuleButtonAction(ActionEvent event) {
        if(CooldownSelected.isSelected()){
            ...
            newRule = new RuleWithSleep(textFieldName.getText(), trigger, action, checkActive.isSelected(), sleepingPeriod);
        }
        
        else{
            newRule = new RuleSingleExecute(textFieldName.getText(), trigger, action, checkActive.isSelected());
        }
        newRule.addToRuleList();
    ...
    private void createAction(ActionEvent event) {
        String ActionType = actionChoiceBox.getValue();
        ActionFactory actionFactory;
        
        switch (ActionType) {
            case "DisplayMessage":
                actionFactory = new DisplayMessageFactory();
                action = actionFactory.create(action);
                break;
            case "PlayAudio":
                actionFactory = new PlayAudioFactory();
                action = actionFactory.create(action);
                break;
            case "AppendStringToFile":
                actionFactory = new AppendStringToFileFactory();
                action = actionFactory.create(action);
                break;
            case "MoveFileToDir":
                actionFactory = new MoveFileToDirFactory();
                action = actionFactory.create(action);
                break;
            case "CopyFileToDir":
                actionFactory = new CopyFileToDirFactory();
                action = actionFactory.create(action);
                break;
            case "DeleteFile":
                actionFactory = new DeleteFileFactory();
                action = actionFactory.create(action);
                break;
            case "ExecuteProgram":
                actionFactory = new ExecuteProgramFactory();
                action = actionFactory.create(action);
                break;
            case "AssignValueToCounter":
                actionFactory = new AssignValueToCounterFactory();
                action = actionFactory.create(action);
                break;
            case "AddValueToCounter":
                actionFactory = new AddValueToCounterFactory();
                action = actionFactory.create(action);
                break;

            default:
                throw new IllegalStateException("Tipo di azione non specificata");
        }
    ...
    private void controlTriggerAndAction() {
        /*
            Verifica che tutti i parametri per la creazione di una regola siano correttamente inseriti
            prima di abilitare il bottone "createRuleButton"
        */
    ...
    private void changeSceneToFrom(VBox to, VBox from){
        //Metodo utilizzato per cambiare la visibilità tra due schermate
        to.setVisible(true);
        to.setDisable(false);
    ...
    private void addTrigger(ActionEvent event) throws IOException {
        //Rende visibili i contenuti della schermata per inserire i parametri per creare i Trigger
        changeSceneToFrom(triggerSelect, contentBase);
    }

    @FXML
    private void addAction(ActionEvent event) {
        //Rende visibili i contenuti della schermata per inserire i parametri per creare le Azioni
    ...
    private void addComplexTrigger(ActionEvent event) {
        /*
            Creazione degli elementi grafici usati per l'inserimento dei parametri necessari per la creazione
            di un trigger complesso il quale è composto da due trigger.
        */
    ...
    private boolean aggiungeTrigger(String boleanTrigger1, String typeTrigger1, String boleanTrigger2, String typeTrigger2, String typeOP, String typeOP2) {
        /*
            Chiama il triggerFactory per la creazione dei Trigger complessi e gestisce il modo in cui questi 
            questi trigger saranno agregati alla struttura composite 
        */
    ...
    private boolean aggiungeSimpleTrigger(String boleanTrigger, String typeTrigger, String typeOP2){
        /*
            Chiama il triggerFactory per la creazione dei Trigger simplici e gestisce il modo in cui questi 
            questi trigger saranno agregati alla struttura composite 
        */
        TriggerFactory triggerFactory = new TriggerFactory();
